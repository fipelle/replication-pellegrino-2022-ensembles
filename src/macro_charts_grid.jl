...
for i in 1:2
    ...
    for j=1:2
        ...
        if j == 1
            subplot_p2 = @pgf Plot3({only_marks, scatter, mark_size="1pt", point_meta="explicit"}, Coordinates(x, y, 1110 .* ones(length(x)), meta=meta_source_scatter));
        elseif j == 2
        ...
        gpr[j][i] = @pgf Axis(
        {
            ...
            ztick_distance = "10",    # set the distance betwen each tick
        },
            subplot_p1,
            subplot_p2,
        );
